...
def showPlayerNamesKey():
    ...
    print("+" + "-" * len(playerKeyString) + "+")
    print("|" + playerKeyString + "|")
    print("+" + "-" * len(playerKeyString) + "+")
...
def checkValidMove(player, piece, dice):
    ...
    if piece == 1:
        if player.getPiece1Pos() + diceToSpaces(dice) == player.getPiece2Pos():
            print("You cannot move here as your other piece is already on that space.")
            return False
    elif piece == 2:
    ...
    if player.getPositions()[piece - 1] == 1 and dice == 4:
            print("You cannot move a piece backwards if it is already at the start.")
            return False
    ...
    if player.getPositions()[piece - 1] == 11 and dice != 4:
            print("You cannot move a piece forwards that is already at the finish.")
            return False
...
def checkForWinner(player):
    if player.getPiece1Pos() == 11 and player.getPiece2Pos() == 11:
        clearScreen()
        winnerMessage = "Game over! %s is the winner!" % activePlayer.getPlayerName()
...
def playGame():
    ...
    while winner == False:
        displayBoard()
        print ("%s's turn..." % activePlayer.getPlayerName())
        dice = rollDice(activePlayer)
        if checkPlayerCanMove(activePlayer, dice) == False:
            print("There is no valid move for %s to make." % activePlayer.getPlayerName())
        else:
            validPiece = False
